...
import radloader.base as base


tag_to_cui = {
              'disease': 'C0012674',  # MeSH category Diseases
              # 'disease': 'C0012634',
              'small_vessel_disease': 'C2733158',
              'atrophy': 'C0333641',
              'haemorrhage': 'C0019080',
              'subdural_haematoma': 'C0018946',
              'subarachnoid_haemorrhage': 'C0038525',
              'microhaemorrhage': 'C0859253',
              'haemorrhagic_transformation': 'C1096400',
              # 'tumour': 'C0027651',   # Neoplasm
              'tumour': 'C0027766',   # Central Nervous System Neoplasms
              'mening_tumour': 'C0025286',
              'metast_tumour': 'C0027627',
              'glioma_tumour': 'C0017638',
              'stroke': 'C0038454',  # Cerebrovascular accident
              'ischaemic_stroke': 'C0948008',  # Ischemic stroke
              'haemorrhagic_stroke': 'C0553692',
              # 'time_recent': 'C0205178',  # acute
              'time_recent': 'C0332185',
              'time_old': 'C0205125',
              'loc_deep': 'C0677546',
              'loc_cortical': 'C0007776'
              }


cui_to_tag = dict(((v, k) for k, v in tag_to_cui.items()))
...
class EdIEDoc(base.Document):
    ...
    def __repr__(self):
        template = '<EdIEDoc %s : %d report, %d conclusion, labels: %r>'
...
class EdIESent(base.Sentence):
    ...
    def spannified_ner_tags(self):
        ...
        while i < len(tags):
            ...
            # Find an entity span
            if ner_type not in ('O', base.Vocab.PAD):
    ...
    # @property
    # def negation_ent_only(self):
    #     return tuple(neg if flag == 'ent' else base.Vocab.PAD
    #                  for neg, flag in zip(self.negation, self.is_entity))
    ...
    def ner_tags_no_bio(self):
        return tuple(tag[2:] if tag not in ('O', base.Vocab.PAD) else tag
    ...
    # def to_xml(self):
    #     elem = ET.Element(EdIEDoc.XML_SENT,
    #                       {EdIEDoc.XML_ID: self.sent_id,
    #                        EdIEDoc.XML_PROCESS: 'yes'})
    #     for i in range(len(self)):
    #         w_attrs = dict()
    #         w_attrs[EdIEDoc.XML_ID] = self.word_ids[i]
    #         w_attrs[EdIEDoc.XML_LEMMA] = self.lemmas[i]
    #         w_attrs[EdIEDoc.XML_POS] = self.pos_tags[i]
    #         token = ET.SubElement(elem,
    #                               EdIEDoc.XML_WORD,
    #                               w_attrs)
    #         token.text = self.tokens[i]
    #     return elem
...
class EdIELabel(object):
    ...
    def to_xml(self):
        ...
        # parts = ET.SubElement(label_xml, EdIEDoc.XML_PARTS)
        # part = ET.SubElement(parts, EdIEDoc.XML_PART)
        # part.set(EdIEDoc.XML_START_WORD, self.span[0])
        # part.set(EdIEDoc.XML_END_WORD, self.span[1])
        # part.text = self.text
